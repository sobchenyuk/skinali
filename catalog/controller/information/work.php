...
class ControllerInformationWork extends Controller {
    ...
    public function index(){
        ...
        //public function info(){
            ...
            $filter_data = array();
            $all_news = $this->model_catalog_news->getNews($filter_data);

            //$data['all_news'] = $all_news;

            for ($i = 0; $i < count($all_news); $i++) {
                if ($all_news[$i]['news_id'] == $news_id) {
                    if ($i + 1 < count($all_news)) $next_new = $all_news[$i + 1];
                    else $next_new = null;
                    if ($i - 1 > 0) $prev_new = $all_news[$i - 1];
                    else $prev_new = null;
                }
            }
            ...
            if ($next_new !== null) {
                $data['next_new'] = array(
                    'title' => $next_new['title'],
                    'href' => $this->url->link('information/news', 'news_id=' . $next_new['news_id'])
                );
            }
            else $data['next_new'] = null;

            if ($prev_new !== null) {
                $data['prev_new'] = array(
                    'title' => $prev_new['title'],
                    'href' => $this->url->link('information/news', 'news_id=' . $prev_new['news_id'])
                );
            }
            else $data['prev_new'] = null;
