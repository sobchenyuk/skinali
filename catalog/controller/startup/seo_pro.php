...
class ControllerStartupSeoPro extends Controller {
	...
	public function rewrite($link) {
		...
		switch ($route) {
			...
			case 'information/news':
				if (isset($data['news_id'])) {
					...
					$data = array();
					if ($this->config->get('config_seo_url_include_path')) {
						//$data['path'] = $this->getPathByNews($tmp['product_id']);
						//if (!$data['path']) return $link;
					}
		...
		if(count($rows) == count($queries)) {
			...
			if ($news == true) $seo_url = 'news'.$seo_url;
			foreach($queries as $query) {
	...
	private function getPathByProduct($product_id) {
		...
		if (!isset($path[$product_id])) {
			...
			$sc_query = $this->db->query("SELECT category_id FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . $product_id . "' AND main_category = 0 LIMIT 1");
			
			//$path[$product_id] = $this->getPathByCategory($query->num_rows ? (int)$query->row['category_id'] : 0);
			$main_cat = $this->getPathByCategory($query->num_rows ? (int)$query->row['category_id'] : 0);
			$sub_cat = $this->getPathByCategory($query->num_rows ? (int)$sc_query->row['category_id'] : 0);

			//$path[$product_id] = $main_cat . '/' . $sub_cat;
			$path[$product_id] = $sub_cat;
	...
	private function getPathByNews($news_id) {
		...
		static $path = null;
		if (!isset($path)) {
			$path = $this->cache->get('news.seopath');
			if (!isset($path)) $path = array();
		}
		...
		if (!isset($path[$product_id])) {
			...
			//$path[$product_id] = $main_cat . '/' . $sub_cat;
			$path[$news_id] = $sub_cat;
			
			$this->cache->set('news.seopath', $path);
		}
