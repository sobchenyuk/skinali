...
class ModelCatalogNews extends Model {
		...
		if ($data['keyword']) {
            if( $this->config->get('smp_sug_is_install') ) {
					...
					foreach( $data['keyword'] as $language_id => $keyword ) {
						if( $keyword != '' )
							$this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'news_id=" . (int)$news_id . "', keyword = '" . $this->db->escape($keyword) . "', smp_language_id = '" . $this->db->escape($language_id) . "'");
					}
				} else {
	...
	public function getNewsStory($news_id) { 
		$query = $this->db->query("SELECT DISTINCT *," . ( $this->config->get('smp_sug_is_install') ? "(SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = 'news_id=" . (int)$news_id . "' AND (smp_language_id IS NULL OR smp_language_id = " . $this->config->get('config_language_id') . ") ORDER BY smp_language_id DESC LIMIT 1 ) AS keyword" : "(SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = 'news_id=" . (int)$news_id . "' LIMIT 1 ) AS keyword" ) . " FROM " . DB_PREFIX . "news n LEFT JOIN " . DB_PREFIX . "news_description nd ON (n.news_id = nd.news_id) WHERE n.news_id = '" . (int)$news_id . "' AND nd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
