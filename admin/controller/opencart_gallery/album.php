...
class ControllerOpencartGalleryAlbum extends Controller { 
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('opencart_gallery/album', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('opencart_gallery/album', 'token=' . $this->session->data['token'] . $url, true),
			'separator' => ' :: '
		);

		$data['opencart_gallery_manager'] = $this->url->link('extension/module/opencart_gallery_manager', 'token=' . $this->session->data['token'], true);

		$data['insert'] = $this->url->link('opencart_gallery/album/insert', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('opencart_gallery/album/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			...
			$data['albums'][] = array(
				...
				'edit'        => $this->url->link('opencart_gallery/album/edit', 'token=' . $this->session->data['token'] . '&album_id=' . $result['album_id'] . $url, true),
				'delete'      => $this->url->link('opencart_gallery/album/delete', 'token=' . $this->session->data['token'] . '&album_id=' . $result['album_id'] . $url, true)
			);
		}
		...
		$pagination->url = $this->url->link('opencart_gallery/album', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('opencart_gallery/album_list', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['album_id'])) {
			$data['action'] = $this->url->link('opencart_gallery/album/insert', 'token=' . $this->session->data['token'], true);
		} else {
			$data['action'] = $this->url->link('opencart_gallery/album/edit', 'token=' . $this->session->data['token'] . '&album_id=' . $this->request->get['album_id'], true);
		}

		$data['cancel'] = $this->url->link('opencart_gallery/album', 'token=' . $this->session->data['token'], true);
		...
		$this->response->setOutput($this->load->view('opencart_gallery/album_form', $data));
	}
