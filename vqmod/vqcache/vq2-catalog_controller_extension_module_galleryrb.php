    ...
    foreach ($results as $result) {
      if (is_file(DIR_IMAGE . $result['image'])) {
        ...
        if(isset($setting['resize'])){
          $thumb = $this->model_tool_galleryimage->resize($result['image'].(isset($result['product_id'])?'::'.$result['product_id']:''), $setting['thumb_width'], $setting['thumb_height']);
        } else {
          $thumb = $this->model_tool_image->resize($result['image'].(isset($result['product_id'])?'::'.$result['product_id']:''), $setting['thumb_width'], $setting['thumb_height']);
        }
        
        $data['galleries'][] = array(
          'title' => htmlspecialchars_decode($result['gallery_image_description'],ENT_QUOTES),
          'thumb' => $thumb,
          'image' => $this->model_tool_image->resize($result['image'].(isset($result['product_id'])?'::'.$result['product_id']:''), $setting['popup_width'], $new_popup_height)
        );
      }
    }
    $data['module'] = $module++;
