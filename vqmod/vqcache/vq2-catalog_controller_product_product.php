	...
	public function index() {
		...
		foreach ($product_categories as $product_category) {
			...
			if ($category_info['parent_id'] === 0) {
				...
				$data['breadcrumbs'][] = array(
					'text' => $category_info['name'],
					'href' => $this->url->link('product/category', 'path=' . $category_info['parent_id'] . '_' . $category_info['category_id'])
				);
			}
		}
		$url = '';
		...
		if ($product_info) {
			...
			foreach ($results as $result) {
				/*$data['images'][] = array(
					'popup' => $this->model_tool_image->resize($result['image'].(isset($result['product_id'])?'::'.$result['product_id']:''), $this->config->get($this->config->get('config_theme') . '_image_popup_width'), $this->config->get($this->config->get('config_theme') . '_image_popup_height')),
