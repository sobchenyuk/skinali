...
class ControllerProductSearch extends Controller {
	...
	public function index() {
		...
		if (isset($this->request->get['search'])) {
			$this->document->setTitle("Вы искали " . $this->request->get['search'] . " | Скинали PrintColor");
		}
		...
		if (isset($this->request->get['search'])) {
			$url .= '&search=' . urlencode(html_entity_decode($this->_decodeSearchKeyword(), ENT_QUOTES, 'UTF-8'));
		}
		...
		if (isset($this->request->get['search'])) {
			$data['heading_title'] = $this->language->get('heading_title') .  ' - ' . $this->_decodeSearchKeyword();
		} else {
			$data['heading_title'] = $this->language->get('heading_title');
		}
		
		$data['search_request'] = $this->request->get['search'];
		...
		if (isset($this->request->get['search']) || isset($this->request->get['tag'])) {
			...
			$colors = array("красный", "оранжевый", "желтый", "зеленый", "голубой", "синий", "фиолетовый", "черный", "белый");
			
			if (in_array($this->request->get['search'], $colors)) {
				...
				$find_by_color = $this->model_catalog_product->getProductsByColor($filter_data);
				$find_all_by_color = $this->model_catalog_product->getAllProductsByColor($color);
				
				$this->load->model('catalog/product');
				$results = array();
				foreach ($find_by_color as $one_product) {
					$product_info = $this->model_catalog_product->getProduct($one_product['product_id']);
					array_push($results, $product_info);
				}
				
				$product_total = count($find_all_by_color);
			}
			else {
			$filter_data = array(
				'filter_name'         => $search,
			...
			/*if (is_numeric($search)) {
				$search = trim($search);
				$product_total = $this->model_catalog_product->getTotalProductsBySku($search);
				$data['total_pro'] = $product_total;
				//$results = $this->model_catalog_product->getProductsBySku($search);
			}*/
